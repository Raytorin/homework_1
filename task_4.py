...
def mid_all_student(all_student, course_student):
    sum_all_mid = 0
    count_all_mid = 0
    all_final_mid = 0
    for correct_student in all_student:
        for courses_correct_student in correct_student.courses_in_progress:
            if courses_correct_student == course_student:
                sum_all_mid += correct_student._middle_grades_student()
                count_all_mid += 1
    all_final_mid = sum_all_mid / count_all_mid
    return print(f'Средний бал за курс по студентам: {all_final_mid}')

def mid_all_lector(all_lector, course_lector):
    sum_all_mid_lector = 0
    count_all_mid_lector = 0
    all_final_mid_lector = 0
    for correct_lector in all_lector:
        for courses_correct_lector in correct_lector.courses_attached:
            if courses_correct_lector == course_lector:
                sum_all_mid_lector += correct_lector._middle_grades_lector()
                print(correct_lector._middle_grades_lector())
                print(sum_all_mid_lector)
                count_all_mid_lector += 1
    all_final_mid_lector = sum_all_mid_lector / count_all_mid_lector
    return print(f'Средний бал за курс по лекторам: {all_final_mid_lector}')
...
cool_lector_3 = Lector('Som', 'Ton')
cool_lector_3.courses_attached += ['Python']
...
best_student_1.rate_lector(cool_lector_3, 'Python', 6)
best_student_2.rate_lector(cool_lector_2, 'Git', 10)
best_student_3.rate_lector(cool_lector_1, 'Python', 5)
best_student_3.rate_lector(cool_lector_3, 'Python', 7)
...
all_students = (best_student_1, best_student_2, best_student_3)
correct_course_student = 'Python'

mid_all_student(all_students, correct_course_student)

all_lector = (cool_lector_1, cool_lector_2, cool_lector_3)
correct_course_lector = 'Python'
mid_all_lector(all_lector, correct_course_lector)